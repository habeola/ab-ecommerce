    ...
    def post(self, *args, **kwargs):
        ...
        try:
            ...
            if form.is_valid():
                ...
                if payment_option == 'S':
                    return redirect("shopping:payment", payment_option='stripe')
                elif payment_option == 'P':
                    return redirect("shopping:payment", payment_option='paypal')
                else:
...
class PaymentView(View):
    def get(self, *args, **kwargs):
        ...
        if order.billing_address:
            ...
            messages.warning(self.request, "You have not added a billing adddres")
            return redirect('shopping:checkout-page')
    ...
    def post(self, *args, **kwargs):
        ...
        amount = int(order.get_total() * 100) # amount is in cent

        try:
            charge = stripe.Charge.create(  
                amount=amount,
                currency="usd",
                source=token #tok_mastercard
            )
             # create payment
            payment = Payment()
        ...
        # yourself an email
            ...
            messages.warning(self.request, "Error occured. We have been notified!")
            return redirect("/")
    ...
    if order_qs.exists():
        ...
        if order.items.filter(item__slug=item.slug).exists():
            ...
            if order_item.quantity > 1:
                ...
                order.items.remove(order_item)
            messages.info(request, "The item was updated")
            return redirect("shopping:order-summary")
        else:
            messages.info(request, "This item was not in your cart")
            return redirect("shopping:product", slug=slug)
